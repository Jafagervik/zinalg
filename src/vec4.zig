...
pub fn Vec4(comptime T: type) type {
    return struct {
        ...
        pub fn init(e0: T, e1: T, e2: T, e3: T) Self {
            return Self{ .e = [_]T{ e0, e1, e2, e3 } };
        }

        pub fn empty() Self {
            return Self{ .e = [_]T{ 0, 0, 0, 0 } };
        }
        ...
        pub fn negate(self: Self) Self {
            return Self.init(-self.e[0], -self.e[1], -self.e[2], -self.e[3]);
        }
        ...
        pub fn addScalar(self: Self, t: T) Self {
            return Self.init(t + self.e[0], t + self.e[1], t + self.e[2], t + self.e[3]);
        }
        ...
        pub fn format(writer: anytype, vec: Self) !void {
            try writer.print("{d} {d} {d} {d}", .{ vec.e[0], vec.e[1], vec.e[2], vec.e[3] });
        }

        /// Directly prints the vector
        pub fn print(vec: Self) void {
            std.debug.print("Vec4: [{d} {d} {d} {d}]\n", .{ vec.e[0], vec.e[1], vec.e[2], vec.e[3] });
        }
...
test "f16" {
    const v: Vec4(f16) = .init(1.0, 2.0, 3.0, 4.0);

    try testing.expectEqual(30.0, v.lengthSquared());
}
...
test "f64" {
    const v: Vec4(f64) = .init(1.0, 2.0, 3.0, 4.0);
